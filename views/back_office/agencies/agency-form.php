...
$js = <<<JS
    ...
    (function() {
      ...
      if (!form) {
        console.warn('BrandForm: form not found', formId);
        return;
      }
    
      // Robust selectors: match inputs by their "name" attribute suffix (works for AR and FormModel)
      var nameInput = form.querySelector('input[name$="[name]"]');
      var slugInput = form.querySelector('input[name$="[url_name]"]');
    
      if (!nameInput || !slugInput) {
        console.warn('BrandForm: inputs not found', { haveName: !!nameInput, haveSlug: !!slugInput });
        return;
      }
